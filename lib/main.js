...
function secondsInMonth( month, year ) {
	...
	if ( arguments.length === 0 ) {
		...
		if ( isDateObject( month ) ) {
			...
			throw new TypeError( format( '1QVAZ,LU', month ) );
		}
	} else {
		if ( !isString( month ) && !isInteger( month ) ) {
			throw new TypeError( format( '1QVAX,LV', month ) );
		}
		if ( !isInteger( year ) ) {
			throw new TypeError( format( '1QV7f,LW', year ) );
		}
		mon = month;
		yr = year;
	}
	if ( isInteger( mon ) && (mon < 1 || mon > 12) ) {
		throw new RangeError( format( '1QVAa,LX', mon ) );
	}
	mon = lowercase( mon.toString() );
	secs = SECONDS_IN_MONTH[ mon ];
	if ( secs === void 0 ) {
		throw new Error( format( '1QVAb,LY', mon ) );
	}
